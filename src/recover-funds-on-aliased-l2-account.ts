...
const recoverFundsOnAliasedL2Account = async () => {
  ...
  if (aliasedSignerBalance.lte(0)) {
    console.warn(
      'Address ' + signerAddress.value + '(Alias: ' + aliasedSignerAddress.value + ') does not have funds on L2'
    );
    return;
  }
  ...
  console.log(
    'Operation to perform: Move ' +
      utils.formatEther(aliasedSignerBalance.toString()) +
      ' ETH from ' +
      aliasedSignerAddress.value +
      ' to ' +
      destinationAddress
  );
  ...
  const inbox = Inbox__factory.connect(l2Network.ethBridge.inbox, baseL2Provider);
