...
export async function findRetryableTxFromL1(
  l1txReceipt: L1TransactionReceipt,
  baseL1Provider: providers.StaticJsonRpcProvider,
  baseL2Provider: providers.StaticJsonRpcProvider
) {
  ...
  if (l1ToL2Messages) {
    for (const l1ToL2Message of l1ToL2Messages) {
      ...
      if (!retryableTxReceipt) {
        console.log(
          'Retryable transaction with hash ' +
            retryableTicketId +
            ' was not found on L2 (if the Ticket was just submitted it might still be pending).'
        );
        return resultObj;
      }
      ...
      if (retryableTxReceipt.status === 0) {
        console.log(
          'Retryable transaction with hash ' +
            retryableTicketId +
            ' FAILED to be created (maybe because the fees paid during the L1 submission was not enough to cover the L2 transaction).'
        );
        return resultObj;
      }
      ...
      if (!l2txReceipt) {
        console.log(
          'L2 transaction with hash ' + l2txHash + ' was not found on L2 (it might not have been executed yet).'
        );
        return resultObj;
      }
  ...
  if (depositMessages) {
    for (const depositMessage of depositMessages) {
      ...
      if (!l2DepositTxReceipt) {
        console.log(
          'Deposit transaction with hash ' + l2DepositTx + ' was not found on L2 (it might not have been executed yet).'
        );
        return resultObj;
      }
...
export async function findRetryableTxFromL2(
  ...
  while (retryableTicketId === '' && l2CurrentToBlock > l2MinSearchableBlock) {
    const redeemEvents = await l2EventFetcher.getEvents(
      ArbRetryableTx__factory,
      contract => contract.filters.RedeemScheduled(null, l2txReceipt.transactionHash),
      {
  ...
  while (l1TxHash === '' && l1CurrentToBlock > l1MinSearchableBlock) {
    const messageDeliveredEvents = await l1EventFetcher.getEvents(
      Bridge__factory,
      contract => contract.filters.MessageDelivered(),
      {
    ...
    const inboxMessageDeliveredEvents = await l1EventFetcher.getEvents(
      Inbox__factory,
    ...
    if (messageDeliveredEvents.length > 0) {
      for (let i = 0; i < messageDeliveredEvents.length; i++) {
        ...
        const currentInboxMessageDeliveredEvent = inboxMessageDeliveredEvents.filter(elem =>
          elem.event.messageNum.eq(currentMessageDeliveredEvent.event.messageIndex)
        )[0];
        if (currentMessageDeliveredEvent && currentInboxMessageDeliveredEvent) {
          try {
            if (isL1ToL2Message) {
              ...
              if (l1ToL2Message && l1ToL2Message.retryableCreationId == retryableTicketId) {
                // FOUND IT !!!
                resultObj.l1ToL2Message = l1ToL2Message;
              ...
              if (depositMessage && depositMessage.l2DepositTxHash == l2txReceipt.transactionHash) {
                // FOUND IT !!!
                resultObj.depositMessage = depositMessage;
                l1TxHash = currentMessageDeliveredEvent.transactionHash;
              }
...
////////////////////
///
export async function findL2ToL1TxFromL2(
  l2TxReceipt: L2TransactionReceipt,
  baseL1Provider: providers.StaticJsonRpcProvider,
  baseL2Provider: providers.StaticJsonRpcProvider
) {
  ...
  if (l2ToL1TxStatus === L2ToL1MessageStatus.EXECUTED) {
    ...
    while (!l1OutboxEvent && l1CurrentFromBlock < l1MaxSearchableBlock) {
      const outboxTxExecutedEvents = await l1EventFetcher.getEvents(
        Outbox__factory,
        contract =>
          contract.filters.OutBoxTransactionExecuted(
