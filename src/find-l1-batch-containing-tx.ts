    ...
    findL1BatchContainingTx(args[0], baseL1Provider, baseL2Provider)
        .then((result) => {
            if (result.l1BlockHash === "") {
                ...
                console.log("Txn " + result.l2TxHash + " has been included in an L1 Batch Block in txn " + result.l1BlockHash + " but it has not been confirmed yet.");
            } else {
                console.log("Txn " + result.l2TxHash + " was included in an L1 Batch Block in txn " + result.l1BlockHash + " which has " + result.confirmations + " confirmations.");
            }
            process.exit(0);
        })
        .catch(error => {
