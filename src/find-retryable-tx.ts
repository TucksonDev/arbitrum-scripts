...
const findRetryableTx = async (txHash: string) => {
    ...
    if (l1txReceipt) {
        ...
        const l1TxSearchResult = await findRetryableTxFromL1(new L1TransactionReceipt(l1txReceipt), baseL1Provider, baseL2Provider);
    ...
    if (l2txReceipt) {
        ...
        const l2TxSearchResult = await findRetryableTxFromL2(new L2TransactionReceipt(l2txReceipt), baseL1Provider, baseL2Provider);
